...
void show() {
    ...
    while (Fi != NULL) {
        if (Fi->In_Start == 0) {
            cout << Fi->In_num << "\t" << Fi->In_Start << "\t" << Fi->In_lengh
                 << "\t" << Fi->In_How << endl;
        } else {
            cout << Fi->In_num << "\t" << Fi->In_Start + 1 << "\t"
                 << Fi->In_lengh << "\t" << Fi->In_How << endl;
        }
        cout << endl;
        Fi = Fi->In_p;
    }
    ...
    while (Uii != NULL) {
        cout << Uii->U_num << "\t" << Uii->U_Start + 1 << "\t" << Uii->U_lengh
             << endl;
        cout << endl;
        if (Uii->U_lengh == 320)
            break;
        ...
        if (Jc[j].time <= i) {  //到达时间小于等于当前时间
            ...
            while (q != NULL) {
                if (q->U_lengh >= Jc[j].lon && q != NULL) {
                    ...
                    if (T->In_Start == 0)
                        cout << "   内存起始地址为:" << T->In_Start << endl
                             << endl;
                    else
                        cout << "   内存起始地址为:" << T->In_Start + 1 << endl
                             << endl;
