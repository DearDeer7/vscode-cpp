...
public:
    ...
    void menu() {
        cout << "请输入进程个数";
        cin >> m;
        procs = vector<process>(m); // 进程请求表proc

        cout << "请输入进程的进程名，进程长度，到达时间，执行时间：" << endl;
        for (int i = 0; i < m; i++) {
            cin >> procs[i].name >> procs[i].length 
                >> procs[i].arri_time >> procs[i].exec_time;
        }
    ...
    void execute(int n) {
        ...
        for (int i = 0; i < TIME_LIMIT; i++) {
            cout << "---------"<< "当前时间：" << i 
                << "----------" << endl;
            if (procs[j].arri_time <= i) {  //到达时间小于等于当前时间
                avail *q = availTab;
                while (q != nullptr) {
                    if (q->length >= procs[j].length) {
                        cout << "进程" << procs[j].name << "内存分配成功，进入内存"
                            << endl;
                        cout << "该进程使用内存大小为:" << procs[j].length << "KB\n";
                        needModify = q->num;

                        procs[j].address = q->begin_addr;
                        procs[j].num = q->num;

                        //修改可用表
                        q->length = q->length - procs[j].length;
                        q->num = ++addNum;
                        q->begin_addr = q->begin_addr + procs[j].length;
                        ...
                        info *tmp = new info(
                            addNum, ti->length - procs[j].length, 
                            ti->begin_addr + procs[j].length,"未分配",ti->next
                        );

                        ti->length = procs[j].length;
                        ti->status = "已分配";
                        ti->next = tmp;
                        ...
                        if (ti->begin_addr == 0)
                            cout << "在内存中起始地址为:" << ti->begin_addr << endl
                                << endl;
                        else
                            cout << "在内存中起始地址为:" << ti->begin_addr + 1 << endl
                                << endl;
            ...
            //判断是否有进程执行结束
            for (int k = 0; k <= j; k++) {
                if (procs[k].exec_time + procs[k].arri_time == i) {
                    finished++;
                    cout << "进程" << procs[k].name << "执行完毕" << endl;
                    info *ti = infoTab;

                    int mergeDown = 0;
                    int mergeUp = 0;
                    while (ti != nullptr) {
                        if (ti->num == procs[k].num) {
                            ti->status = "未分配";
...
private:
    ...
    const int INIT_ADDR = 0; // 内存初始开始地址
